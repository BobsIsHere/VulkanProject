...
private:
    ...
    void createSwapChain() 
    {
        ...
        if (vkCreateSwapchainKHR(m_pVulkanDevice->GetDevice(), &createInfo, nullptr, &swapChain) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create swap chain!"); 
        }

        vkGetSwapchainImagesKHR(m_pVulkanDevice->GetDevice(), swapChain, &imageCount, nullptr);
        swapChainImages.resize(imageCount);
        vkGetSwapchainImagesKHR(m_pVulkanDevice->GetDevice(), swapChain, &imageCount, swapChainImages.data());
    ...
    void createRenderPass() 
    {
        ...
        if (vkCreateRenderPass(m_pVulkanDevice->GetDevice(), &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS)
        {
    ...
    void createDescriptorSetLayout() 
    {
        ...
        if (vkCreateDescriptorSetLayout(m_pVulkanDevice->GetDevice(), &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS)
        {
    ...
    void createGraphicsPipeline() 
    {
        ...
        if (vkCreatePipelineLayout(m_pVulkanDevice->GetDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
        {
        ...
        if (vkCreateGraphicsPipelines(m_pVulkanDevice->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo,
            nullptr, &graphicsPipeline) != VK_SUCCESS) 
        {
            throw std::runtime_error("failed to create graphics pipeline!");
        }

        vkDestroyShaderModule(m_pVulkanDevice->GetDevice(), fragShaderModule, nullptr);
        vkDestroyShaderModule(m_pVulkanDevice->GetDevice(), vertShaderModule, nullptr);
    }
    ...
    {
        std::vector<VkDescriptorSetLayout> layouts(utils::MAX_FRAMES_IN_FLIGHT, descriptorSetLayout);
    ...
    void drawFrame() 
    {
        ...
        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(m_pVulkanDevice->GetDevice(), swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
    ...
    void cleanupSwapChain() 
    {
        ...
        vkDestroySwapchainKHR(m_pVulkanDevice->GetDevice(), swapChain, nullptr);
    }
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) 
    {
        ...
        if (vkCreateShaderModule(m_pVulkanDevice->GetDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
        {
